...
# File to delete data where t > 60
...
def compute_numerical_jerk(t:np.array, vx:np.array, vy: np.array, vz:np.array):
...
def compute_arc_length(t:np.array, vx:np.array, vy: np.array, vz:np.array):
    n = len(t)
...
def compute_score(collisions_path:str):
    ...
    try:
        ...
        green_balls = collisions_file[collisions_file[' Ball Color'] == 'Green'].shape[0]
...
for user in os.listdir(data_path):
    ...
    experiment_results = pd.read_csv(os.path.join(user_path, 'ExperimentResults.csv'), 
                                         index_col = 'CompletedOrder')

    for b in boards:
        trajectory_path = os.path.join(positions_path, f'Board{b}Trial1.csv')
        collisions_file = os.path.join(collisions_path, f'Board{b}Trial1.csv')
    ...
    experiment_results['NJS'] = experiment_results['Identifier'].map(metric_information_njs).combine_first(experiment_results['NJS']) 
    
    experiment_results['Avg. Speed'] = experiment_results['Identifier'].map(metric_information_avg_speed).combine_first(experiment_results['Avg. Speed'])
     
    experiment_results['CompletedTime'] = experiment_results['Identifier'].map(metric_information_time).combine_first(experiment_results['CompletedTime']) 
    
    experiment_results['Score'] = experiment_results['Identifier'].map(metric_information_score).combine_first(experiment_results['Score']) 
