"""Module for generating the graphs using the data. 

Includes helper classes like Board and Patient for storing the information.
"""
...
class Board:
    def __init__(self, id:str, green_balls:int, figure:str, size:str):
        self.id = id
...
class Patient:
    def __initc(self, name: str, age: int, genre: str, group: str):
        self.name = name
        self.age = age
        self.genre = genre
        self.group = group
...
class UserDataBase:
    ...
    def load_data(self):
        ...
        for user in user_names:
            ...
            p = (
                patient.group == "JOV"
                or patient.group == "NOR"
                or patient.group == "DCL"
            )
            q = ( (patient.group == "Adulto Sano") or
                (patient.group == "Control Sano") )
    ...
    def _show_groups(self):
        print("AJS")
        for patient in self.group_ajs:
            print(patient)

        print("AMD")
        for patient in self.group_ams:
            print(patient)

        print("AMD")
        for patient in self.group_amd:
            print(patient)
        ...
        if patient_name in self.patients:
            for board_id in board_ids:
                ...
                positions_path = os.path.join(
                    self.path_database, patient_name, "Positions"
                )
                ...
                if pathlib.Path(path_to_board).exists():
                    ...
                    print(f"El paciente {patient_name} no completó el tablero {board_id}")
        else:
    ...
    def generate_comparing_positions_name(self, patient_names: list[str], board_ids: list[str]):
        """
            ...
            if board_id in self.boards:
                ...
                for patient_name in patient_names:
                    ...
                    if patient_name in self.patients:
                        ...
                        print(
                            "    Error. El paciente no existe. No es añadido a la gráfica."
                        )
                    ...
                    save_directory = os.path.join(
                        os.getcwd(), "Multiple Trajectories Figures"
                    )
    ...
    def generate_all_experiment_results(self, all_data_set=True, by_groups=True):
        ...
        for patient_name in self.patients:
            ...
            experiment_results_path = os.path.join(
                self.path_database, patient_name, "ExperimentResults.csv"
            )
            ...
            df = df.rename({"Avg. Speed": "Avg.Speed", "Avg. Acc": "Avg.Acc"}, axis=1)
    ...
    def generate_metric_graph(self, m: str, t: str):
        ...
        if not (os.path.exists(file_path)):
            print(f"El archivo {file_path} no existe. Generandolo.")
        ...
        for g in self.groups_ids:
            ...
            df = pd.read_csv(
                f"{g}-AllExperimentResults.csv", index_col=1
            )  # Column at position 1 is the index(Board Identifier)
            ...
            if m not in df.columns:
                ...
                # Select the metric 'm' and Change the column label (Hay algo mal)
        ...
        if m == "Score" or m == "CompletedTime":
            ...
            axes.hlines(
                y_max_possible_value,
                [1],
                3,
                lw=2,
                colors="r",
                label="Valor máximo posible",
            )
    ...
    def generate_collision_graph(self, t: str):
        ...
        collision_by_ball_type = {
            "Verde": [
                [
                    collision_information["Verde"]
                    for group, collision_information in collisions_groups.items()
                ],
                "tab:green",
            ],
            "Roja": [
                [
                    collision_information["Roja"]
                    for group, collision_information in collisions_groups.items()
                ],
                "tab:red",
            ],
        ...
        for ball_color, information in collision_by_ball_type.items():
            ...
            collision_avg_by_group = np.array(
                [
                    round(collisions_by_group[0][0]),
                    round(collisions_by_group[1][0]),
                    round(collisions_by_group[2][0]),
                ]
            )

            std_by_group = np.array(
                [
                    collisions_by_group[0][1],
    ...
    def _preprocess_information(self, path_to_board: str) -> np.ndarray:
    ...
    def _plot_information(
        ...
        *,
        ...
        if show_name:
            ...
            axes.set_title(
                f"Trayectoria de {self.patients[patient_name].group} tablero {id}"
            )
        ...
        axes.plot(
            position_plot[-1, 0], position_plot[-1, 2], position_plot[-1, 1], "rx"
        )
        ...
        if len(patient_name_splitted) > 1:
            ...
            if show_name:
                figure_name = (
                    f"{patient.group}EndEffectorPosition{patient_name_splitted}{id}"
                )
            else:
                figure_name = f"{patient.group}EndEffectorPosition{id}"
        ...
        if save_graph:
            ...
            fig.savefig(
                os.path.join(save_directory, f"{figure_name}.{self.extension}"),
                bbox_inches="tight",
                pad_inches=0.3,
            )
    ...
    def _load_collision_information_all_boards(self, user_path: str, patient: Patient):
        ...
        for board_id in self.boards:
            ...
            collision_file = open(
                os.path.join(user_collisions_path, f"Board{board_id}.csv")
            )
    ...
    def _generate_collisions_statistics(self, t: str):
        ...
        for g in self.groups_ids:
            ...
            group_collisions = {
                "Verde": [np.average(green_collisions_np), np.std(green_collisions_np)],
    ...
    def _plot_on_axes(self, axes, position_plot: np.ndarray):
        # Plot the graph
        x = position_plot[:, 0]
        y = position_plot[:, 1]
        z = position_plot[:, 2]
